...
$(document).on("mouseup mouseenter mouseleave mouseout touchstart touchend touch", ".canvasSign", function () {
    var $this = $(this);
    var data = canvasArray[$this.attr('id')].toDataURL();;
...
function runAllForms() {
    ...
    $(".datetimepicker").on("dp.change", function (e) {
        $me = $(this);
        if ($me.data("child")) {
            $me.data('DateTimePicker').maxDate($('#' + $me.data("child")).val());
        }
        if ($me.data("parent")) {
            $me.data('DateTimePicker').minDate($('#' + $me.data("parent")).val());
        }
    });
    ...
    try {
        ...
        if (nav_elems.length > 0) {
            nav_elems.each(function () {
                ...
                if ($.bread_crumb.find("li:last-child").text() != 'defined')
