...
{
    public class BaseWebSocketChannelBlackListUsersController : BaseController
    {
        [IsView]
        public ActionResult Index(int id = 0, string from = "")
        {
            ...
            ViewBag.from = from;
            ViewBag.tableName = "BaseWebSocketChannelBlackListUsers";
        ...
        public JsonResult data(FormCollection form, string from = "", string filter = "")
        {
            ...
            table.TableName = "[BaseWebSocketChannelBlackListUsers]";
            table.PrimaryKey = "Id";
            ...
            table.ReplacedText.Add("UserId", ("User Id"));
            table.ReplacedText.Add("WebSocketChannelId", ("Web Socket Channel Id"));
            table.SearchCondition = " Id like '%{0}%'  OR  (select top 1 username from [BaseUser] where ID=UserId) like '%{0}%'  OR  (select top 1 Name from [BaseWebSocketChannels] where id=WebSocketChannelId) like '%{0}%' ";
            table.VarName = "csBaseWebSocketChannelBlackListUsers" + from.Split(':')[0];
            table.ReferenceText = "WebSocketChannelBlackListUsers";
            table.Links.Add(new CsTableLink() { ButtonType = URLHelper.buttonType("BaseWebSocketChannelBlackListUsers"), Href = URLHelper.getActionUrlRelation("BaseWebSocketChannelBlackListUsers", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil, Extra = extraRelation });
            table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("BaseWebSocketChannelBlackListUsers", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
        ...
        public FileResult getCSV(string from = "")
        {
            ...
            table.ReplacedText.Add("WebSocketChannelId", ("Web Socket Channel Id"));
            table.SearchCondition = " Id like '%{0}%'  OR  (select top 1 username from [BaseUser] where ID=UserId) like '%{0}%'  OR  (select top 1 Name from [BaseWebSocketChannels] where id=WebSocketChannelId) like '%{0}%' ";
            table.VarName = "csBaseWebSocketChannelBlackListUsers";
            table.ReferenceText = "WebSocketChannelBlackListUsers";
            table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.button, Href = URLHelper.getActionUrl("BaseWebSocketChannelBlackListUsers", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil });
            table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("BaseWebSocketChannelBlackListUsers", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
            ...
            wb.Worksheets.Add(table.Data, "Web Socket Channel Black List Users");
            using (var memoryStream = new MemoryStream())
            {
                wb.SaveAs(memoryStream);
                byte[] byteInfo = memoryStream.ToArray();
                return File(byteInfo, System.Net.Mime.MediaTypeNames.Application.Octet, String.Format("Web Socket Channel Black List Users SMSChat {0}.xlsx", DateTime.Now.ToShortDateString()));
            }
        }
        ...
        public JsonResult getAll(FormCollection form, string columns = "['*']", string @operator = "AND")
        {
            ...
            DataTable data = Helper.getData($"SELECT [{string.Join("],[", sel)}] FROM [BaseWebSocketChannelBlackListUsers] {whereCondition} ".Replace("[*]", "*"), db);
