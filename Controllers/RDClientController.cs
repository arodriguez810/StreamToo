...
{
    public class RDClientController : BaseController
    {
[IsView]
        public ActionResult Index(int id = 0, string from = "")
        {
             ...
             ViewBag.from = from;
             ViewBag.tableName = "RDClient";
             return PartialView(new RDClient());
        }
[IsView]
        public ActionResult Form(int id=0, string from = "")
        {
ViewBag.relations = db.VWISRElations.Where(d => d.PK_Table == "RDClient").ToList();
            ViewBag.from = from;
            if (id == 0)
                return PartialView(new RDClient(){ active=true, creationDate = DateTime.Now});
            else
            {
                RDClient model = db.RDClients.Find(id);
                return PartialView(model);
            }
        }
[HttpPost]
[ValidateInput(false)]
        public JsonResult Save(RDClient model, FormCollection form)
        {
try
            {
            if (model.id != 0)
            ...
            {
                return Json(new { Message = Helper.ModeralException(ex).Replace("@table", "Client") });
            }
            }
	...
	RDClient model = db.RDClients.Find(id);
                if (model != null)
                {
                    if (model.active)
                    {
                        ...
                        if (validation.BoolValue)
                            return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
                        return Json(new { id = model.id, MessageSucess = "That Client Inactive successfully." }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                ...
                model.active = !model.active;
                        ...
                        if (validation.BoolValue)
                            return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
                        return Json(new { id = model.id, MessageSucess = "That Client Active successfully." }, JsonRequestBehavior.AllowGet);
                    }
                }
            ...
            {
                return Json(new { Message = Helper.ModeralException(ex).Replace("@table", "Client") }, JsonRequestBehavior.AllowGet);
            }
        }
...
if(model != null){                    BoolString validation = model.BeforeDelete(db);
                    if (validation.BoolValue)
                        return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
	db.RDClients.Remove(model);
	db.SaveChanges();
                    validation = model.AfterDelete(db);
                
                    if (validation.BoolValue)
                        return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
                return Json(new { id = model.id, MessageSucess = "That Client deleted successfully." }, JsonRequestBehavior.AllowGet);
            }
return Json(new { Message = "This record no longer exists" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
...
table.ReplacedText.Add("device", ("Device")); 
	...
	table.Links.Add(new CsTableLink() { ButtonType = URLHelper.buttonType("RDClient"), Href = URLHelper.getActionUrlRelation("RDClient", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil, Extra = extraRelation });
table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.functionButton, Href = URLHelper.getActionUrl("RDClient", "ToggleActive"), Title = "Active/Inactive", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_asterisk });
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("RDClient", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
...
table.ReplacedText.Add("device", ("Device")); 
	table.SearchCondition = " id like '%{0}%'  OR  active like '%{0}%'  OR  creationDate like '%{0}%'  OR  email like '%{0}%'  OR  name like '%{0}%'  OR  lastName like '%{0}%'  OR  birthDate like '%{0}%'  OR  location like '%{0}%'  OR  device like '%{0}%' ";
	table.VarName = "csRDClient";
	table.ReferenceText = "Client";
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.button, Href = URLHelper.getActionUrl("RDClient", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil });
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("RDClient", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
...
{ 
	...
	wb.Worksheets.Add(table.Data, "Client");
	using (var memoryStream = new MemoryStream())
	{
		wb.SaveAs(memoryStream);
		byte[] byteInfo = memoryStream.ToArray();
		return File(byteInfo, System.Net.Mime.MediaTypeNames.Application.Octet, String.Format("Client StreamToo {0}.xlsx", DateTime.Now.ToShortDateString()));
	}
}
[HttpGet]
        [AllowAnonymous]
        public JsonResult getAll(FormCollection form, string columns = "['*']", string @operator = "AND")
        {
            ...
            DataTable data = Helper.getData($"SELECT [{string.Join("],[", sel)}] FROM [RDClient] {whereCondition} ".Replace("[*]", "*"), db);
...
adds.Add(Helper.Regular(db, item, "RDClientChannel", $" where [client_client] = '@id@'"));
        ...
        [HttpPut]
        [AllowAnonymous]
        public JsonResult create(RDClient model)
        {
            BoolString validation = model.BeforeSave(db);
            if (validation.BoolValue)
                return Json(new { Message = validation.StringValue });
