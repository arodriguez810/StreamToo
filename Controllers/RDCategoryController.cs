...
{
    public class RDCategoryController : BaseController
    {
[IsView]
        public ActionResult Index(int id = 0, string from = "")
        {
             ...
             ViewBag.from = from;
             ViewBag.tableName = "RDCategory";
             return PartialView(new RDCategory());
        }
[IsView]
        public ActionResult Form(int id=0, string from = "")
        {
ViewBag.relations = db.VWISRElations.Where(d => d.PK_Table == "RDCategory").ToList();
            ViewBag.from = from;
            if (id == 0)
                return PartialView(new RDCategory(){ active=true, creationDate = DateTime.Now});
            else
            {
...
[HttpPost]
[ValidateInput(false)]
        public JsonResult Save(RDCategory model, FormCollection form)
        {
try
            {
            if (model.id != 0)
            ...
            else
            {
                ...
                return Json(new { id = model.id, MessageSucess = "That Category saved successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { Message = Helper.ModeralException(ex).Replace("@table", "Category") });
            }
            }
...
if(model != null){                    BoolString validation = model.BeforeDelete(db);
                    if (validation.BoolValue)
                        return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
	db.RDCategories.Remove(model);
	db.SaveChanges();
                    validation = model.AfterDelete(db);
                
                    if (validation.BoolValue)
                        return Json(new { Message = validation.StringValue }, JsonRequestBehavior.AllowGet);
                return Json(new { id = model.id, MessageSucess = "That Category deleted successfully." }, JsonRequestBehavior.AllowGet);
            }
return Json(new { Message = "This record no longer exists" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
...
table.ReplacedText.Add("description", ("Description")); 
	...
	table.VarName = "csRDCategory" + from.Split(':')[0];
	table.ReferenceText = "Category";
	table.Links.Add(new CsTableLink() { ButtonType = URLHelper.buttonType("RDCategory"), Href = URLHelper.getActionUrlRelation("RDCategory", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil, Extra = extraRelation });
table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.functionButton, Href = URLHelper.getActionUrl("RDCategory", "ToggleActive"), Title = "Active/Inactive", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_asterisk });
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("RDCategory", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
...
table.ReplacedText.Add("description", ("Description")); 
	table.SearchCondition = " id like '%{0}%'  OR  active like '%{0}%'  OR  creationDate like '%{0}%'  OR  name like '%{0}%'  OR  description like '%{0}%' ";
	table.VarName = "csRDCategory";
	table.ReferenceText = "Category";
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.button, Href = URLHelper.getActionUrl("RDCategory", "Form"), Title = "Edit", Css = "btn btn-warning btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_pencil });
	table.Links.Add(new CsTableLink() { ButtonType = CsTableLinkType.deleteButton, Href = URLHelper.getActionUrl("RDCategory", "Delete"), Title = "Delete", Css = "btn btn-danger btn-circle refreshTable", Icon = BaseUIIconText.fa.fa_fa_trash_o });
...
{ 
	...
	wb.Worksheets.Add(table.Data, "Category");
	using (var memoryStream = new MemoryStream())
	{
		wb.SaveAs(memoryStream);
		byte[] byteInfo = memoryStream.ToArray();
		return File(byteInfo, System.Net.Mime.MediaTypeNames.Application.Octet, String.Format("Category StreamToo {0}.xlsx", DateTime.Now.ToShortDateString()));
	}
}
[HttpGet]
        [AllowAnonymous]
        public JsonResult getAll(FormCollection form, string columns = "['*']", string @operator = "AND")
        {
            ...
            DataTable data = Helper.getData($"SELECT [{string.Join("],[", sel)}] FROM [RDCategory] {whereCondition} ".Replace("[*]", "*"), db);
...
adds.Add(Helper.Regular(db, item, "RDChannel", $" where [category_category] = '@id@'"));
        ...
        [HttpPut]
        [AllowAnonymous]
        public JsonResult create(RDCategory model)
        {
            BoolString validation = model.BeforeSave(db);
            if (validation.BoolValue)
                return Json(new { Message = validation.StringValue });
        ...
        [HttpPost]
        [AllowAnonymous]
        public JsonResult update(int id, RDCategory model)
        {
            BoolString validation = model.BeforeEdit(db);
            if (validation.BoolValue)
                return Json(new { Message = validation.StringValue });
